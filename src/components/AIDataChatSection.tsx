...
function ThinkingIndicator({ text, isExpanded, onToggle }: { text: string; isExpanded: boolean; onToggle: () => void }) {
  return (
    <div className="mb-4">
      <button onClick={onToggle} className="flex items-center gap-2 text-[14px] text-white/60 hover:text-white/80 transition-colors font-light">
        <motion.div animate={{ rotate: isExpanded ? 90 : 0 }} transition={{ duration: 0.2 }}>
          <ChevronRight className="w-4 h-4" />
        </motion.div>
        <div className="flex items-center gap-2">
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}>
            <Sparkles className="w-4 h-4 text-[#006EEE]" />
          </motion.div>
          <span>Thinking...</span>
        </div>
      </button>
      <AnimatePresence>
        {isExpanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <p className="text-[14px] text-white/40 mt-2 ml-6 italic font-light">{text}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function renderMarkdownBold(text: string) {
  ...
  return parts.map((part, i) => i % 2 === 1 ? <strong key={i} className="font-normal text-white">{part}</strong> : part);
}
    ...
    <div className="space-y-3">
      {contentBlocks.slice(0, visibleItems).map((item, idx) => (
        <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          {item.type === "heading" && <h3 className="text-[18px] font-normal text-white">{item.text}</h3>}
          {item.type === "subheading" && <h4 className="text-[16px] font-normal text-white mt-4">{item.text}</h4>}
          {item.type === "paragraph" && <p className="text-white/90 font-light text-[14px]">{renderMarkdownBold(item.text || "")}</p>}
          {item.type === "bullet" && (
            <div className="flex items-start gap-2 ml-2">
              <span className="text-[#006EEE] mt-1.5">•</span>
              <p className="text-white/90 font-light text-[14px]">{renderMarkdownBold(item.text || "")}</p>
            </div>
          )}
          {item.type === "table" && item.data && (
            <div className="bg-white/5 rounded-xl p-3 my-3">
              <table className="w-full text-[14px]">
                <tbody>
                  {item.data.map((row, rowIdx) => (
                    <tr key={rowIdx} className="border-b border-white/10 last:border-0">
                      <td className="py-2 text-white/70 font-light">{row.metric}</td>
                      <td className="py-2 text-right font-mono text-white font-normal">{row.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {startStreaming && visibleItems < contentBlocks.length && (
        <motion.span animate={{ opacity: [1, 0] }} transition={{ duration: 0.5, repeat: Infinity }} className="inline-block w-2 h-4 bg-[#006EEE] ml-1" />
      )}
    </div>
  );
}
...
export function AIDataChatSection() {
  ...
  useEffect(() => {
    if (isInView && !hasStartedRef.current) { hasStartedRef.current = true; startConversation(); }
  }, [isInView]);
    ...
    <section ref={sectionRef} className="py-16 md:py-24 bg-white overflow-hidden">
      <div className="mx-auto px-4 md:px-6 lg:px-8 max-w-6xl">
        <div className="text-center mb-12">
          ...
          <h2 className="text-[32px] md:text-[40px] font-light leading-[120%] text-gradient-4 mb-4">Talk to your data</h2>
          ...
          <div className="rounded-2xl overflow-hidden shadow-lg border border-gray-100">
            <div className="flex h-[500px] md:h-[550px]">
              <div className="hidden md:flex flex-col w-64 bg-[#171717] border-r border-white/10">
                <div className="p-3">
                  <button className="flex items-center gap-2 w-full px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-white/90 text-[14px] font-light transition-colors">
                    <Plus className="w-4 h-4" />New Chat
                  </button>
                </div>
                <div className="px-3 pb-3">
                  <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 text-white/50 text-[14px] font-light">
                    <Search className="w-4 h-4" />Search Chats
                  </div>
                </div>
                <div className="flex-1 overflow-y-auto px-2">
                  <div className="mb-4">
                    <p className="text-[12px] text-white/40 px-2 py-1 font-light">Today</p>
                    {todayChats.map(chat => (
                      <button key={chat.id} className={`flex items-center gap-2 w-full px-3 py-2 rounded-xl text-[14px] font-light text-left transition-colors ${chat.id === 1 ? "bg-white/10 text-white" : "text-white/70 hover:bg-white/5"}`}>
                        <MessageSquare className="w-4 h-4 shrink-0" /><span className="truncate">{chat.title}</span>
                      </button>
                    ))}
                  </div>
                  <div className="mb-4">
                    <p className="text-[12px] text-white/40 px-2 py-1 font-light">Yesterday</p>
                    {yesterdayChats.map(chat => (
                      <button key={chat.id} className="flex items-center gap-2 w-full px-3 py-2 rounded-xl text-[14px] font-light text-left text-white/70 hover:bg-white/5 transition-colors">
                        <MessageSquare className="w-4 h-4 shrink-0" /><span className="truncate">{chat.title}</span>
                      </button>
                    ))}
                  </div>
              ...
              <div className="flex-1 flex flex-col bg-[#212121]">
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                  <div className="flex items-center gap-2">
                    <span className="text-white font-normal text-[16px]">Obenan AI</span>
                      ...
                      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                        <div className="flex justify-end">
                          <div className="bg-[#2f2f2f] rounded-2xl px-4 py-3 max-w-[80%]">
                            <p className="text-white font-light text-[14px]">{userQuery}</p>
                          </div>
                        </div>
                        <div className="flex gap-3">
                          <div className="w-8 h-8 rounded-full bg-[#006EEE] flex items-center justify-center shrink-0">
                ...
                <div className="p-4 border-t border-white/10">
                  <div className="flex items-center gap-2 bg-[#2f2f2f] rounded-full px-4 py-3">
                    <input type="text" value={inputValue} readOnly placeholder="Ask a question about your network..." className="flex-1 bg-transparent text-white placeholder:text-white/40 outline-none text-[14px] font-light" />
                    <button className="w-8 h-8 rounded-full bg-[#006EEE] flex items-center justify-center hover:opacity-90 transition-opacity">
